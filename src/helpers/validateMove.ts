  ...
  switch (type) {
    ...
    case PieceTypes.ROOK:
      for (let positiveRow = 1; positiveRow < 8 - row; positiveRow++) {
        if (
          isInsideBoard(row + positiveRow, col) &&
          !hasPiece(row + positiveRow, col)
        ) {
      ...
      for (let negativeRow = 1; negativeRow <= row; negativeRow++) {
        if (
          isInsideBoard(row - negativeRow, col) &&
          !hasPiece(row - negativeRow, col)
        ) {
      ...
      for (let positiveCol = 1; positiveCol < 8 - col; positiveCol++) {
        if (
          isInsideBoard(row, col + positiveCol) &&
          !hasPiece(row, col + positiveCol)
        ) {
      ...
      for (let negativeCol = 1; negativeCol <= col; negativeCol++) {
        if (
          isInsideBoard(row, col - negativeCol) &&
          !hasPiece(row, col - negativeCol)
        ) {
