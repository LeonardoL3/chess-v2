...
export function validateMove(
	{chessboard, selectedSquare, playerColor}: IValidateMoveProps
	...
	const { position: { row: currRow, col: currCol } } = selectedSquare
	...
	for (const moves of POSSIBLE_POSITIONS_RELATIVE_TO_SELECTED_SQUARE[type]) {
		for (const { row, col, isFirstMoveRequired, isOnlyToAttack } of moves[color]) {
			...
			const positionOnChessNotation = convertPosition(
				[possibleRowPosition, possibleColPosition]
			) as PiecePositions

			const hasPiece = checkIfHasPiece(possibleRowPosition, possibleColPosition)
			const isPieceOfSameColor = checkIfPieceIsOfSameColor(possibleRowPosition, possibleColPosition)
