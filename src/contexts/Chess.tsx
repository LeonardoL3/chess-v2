...
const EMPTY_CHESSBOARD = Array.from(
    {length: 8}, (_, rowIdx) => Array.from({length: 8}, (_, colIdx) => mountSquare(rowIdx, colIdx))
)

console.log(EMPTY_CHESSBOARD[1][6])
...
export function ChessContextWrapper({children}: {children: ReactNode}) {
    ...
    const [previousSelectedSquare, setPreviousSelectedSquare] = useState<ISquareConfig | undefined>(undefined)
    ...
    const startRound = (selectedSquare: ISquareConfig) => {
        switch(state) {
            ...
            case States.TO_EXECUTE:
                ...
                if (selectedSquare.positionOnChessNotation == previousSelectedSquare?.positionOnChessNotation) {
                ...
                setMetrics(prevState => ({
                    ...prevState,
                    player: prevState.player === PieceColors.WHITE ? PieceColors.BLACK : PieceColors.WHITE,
