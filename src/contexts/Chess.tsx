...
import { ISquareContext, Colors, States, SquareColors, IPiece } from '@/helpers/types';
...
export function ChessContextWrapper({ children }: { children: ReactNode }) {
  ...
  const [currentlySelectedPieceToMove, setCurrentlySelectedPieceToMove] = useState<
    ISquareContext | undefined
  >(undefined);
  ...
  const startRound = (selectedSquare: ISquareContext) => {
    switch (state) {
      case States.TO_VALIDATE:
        ...
        setChessboard(prevChessboard => prevChessboard.map(
			row => row.map(
				square => {
      ...
      case States.TO_EXECUTE:
        ...
        if (
          ...
          setChessboard(prevState => prevState.map(
			row => row.map(
				square => ({
					...square,
					isAttackable: false,
        ...
        setChessboard(prevChessboard => prevChessboard.map(
			(row, rowIdx) => row.map(
				(square, colIdx) => {
					const { row: currentRow, col: currentCol } = currentlySelectedPieceToMove?.position as ISquareContext["position"]
					...
					} else if (rowIdx === selectedSquare.position.row && colIdx === selectedSquare.position.col) {
						return {
							...square,
							isAttackable: false,
							isPossibleMove: false,
