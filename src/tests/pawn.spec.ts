...
describe("Pawn validation", () => {
    ...
    it('should handle invalid moves', () => {
        ...
        expect(consoleErrorMock).toHaveBeenCalledWith('Invalid move: square has no piece!')

    })
    it('should move the only the correct piece color', () => {
        const selectedSquare = fakeChessboard[1][6] // white pawn
        const [validatedChessboard, state] = validateMove(PieceColors.BLACK, fakeChessboard, selectedSquare)

        expect(fakeChessboard).toEqual(validatedChessboard)
        expect(state).toBe(States.TO_VALIDATE)
        expect(consoleErrorMock).toHaveBeenCalledOnce()
        expect(consoleErrorMock).toHaveBeenCalledWith('Invalid Move: you can only move black pieces!')
    })
    it('should move execute pawn move', () => {
        const selectedSquare = fakeChessboard[1][6] // white pawn
        const [validatedChessboard, state] = validateMove(PieceColors.WHITE, fakeChessboard, selectedSquare)
        const randomAttackableSquare = validatedChessboard[1].find(cb => cb.isAttackable) as ISquareConfig

        expect(selectedSquare.piece).toBeTruthy()
        expect(randomAttackableSquare).toBeTruthy()
        expect(state).toBe(States.TO_EXECUTE)

        const { position: {row, col} } = selectedSquare
        const { position: {row: attackableRow, col: attackableCol} } = randomAttackableSquare
        ...
        expect(executedChessboard[attackableRow][attackableCol].piece?.type).toBe(selectedSquare.piece?.type)
        expect(executedChessboard[attackableRow][attackableCol].piece?.color).toBe(selectedSquare.piece?.color)
        expect(executedChessboard[attackableRow][attackableCol].piece?.isFirstMove).toBe(false)
